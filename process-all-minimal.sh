#!/bin/bash
# Process all split JSON files using the minimal processor
...
FILE_COUNT=$(ls -1 "$SPLIT_DIR"/*.json 2>/dev/null | wc -l | tr -d ' ')
...
for FILE in "$SPLIT_DIR"/*.json; do
  ...
  # Process file with minimal script
  php minimal-process.php "$FILE"
...
echo ""
echo "All files processed. Check database for results." 